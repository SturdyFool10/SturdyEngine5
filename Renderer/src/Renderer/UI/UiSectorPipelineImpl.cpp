...
namespace SFT::UI {
    ...
    /// Destroys the UI sector frame resources identified by the supplied parameters.
    ///
    /// @param device Device used or affected by the operation.
    /// @param resources `resources` value used by the operation.
    ///
    /// @note This function does not throw exceptions.
    void destroy_ui_sector_frame_resources(RHI::RhiDevice &device, UiSectorFrameResources &resources) noexcept {
        for (const UiSectorDrawBatch::BoundGroup &group : resources.bind_groups) {
            if (group.handle) {
    ...
    namespace {
        ...
        /// @note This function does not throw exceptions.
        [[nodiscard]] bool same_instance(const UiSectorInstance &lhs, const UiSectorInstance &rhs) noexcept {
            return std::memcmp(&lhs, &rhs, sizeof(UiSectorInstance)) == 0;
        }
        ...
        /// Creates an error result describing the supplied UI sector failure.
        ///
        /// @param message Text consumed by the operation.
        ///
        /// @return Returns the value produced by the operation.
        /// @note This function has no separate failure status; exceptions raised by operations it invokes propagate to the caller.
        [[nodiscard]] Core::GraphicsBackendError ui_sector_error(string message) {
    ...
    Core::RendererExpected<UiSectorPipeline> UiSectorPipeline::create(
        ...
        slang::ShaderVariantCache shader_cache{
            slang::ShaderSource::from_file("Shaders/ui_sector.slang", "ui_sector"),
            options,
            slang::ShaderCompiler{},
            enable_shader_disk_cache};
        auto shader = shader_cache.get_or_compile_base();
        if (!shader) {
            return unexpected(ui_sector_error("compile ui_sector shader failed: " + shader.error().message + "\n" + shader.error().diagnostics));
        }

        UiSectorPipeline pipeline;

        auto vertex_code = shader->entry_point_code("vertexMain", shader_target->slang_target.format);
        if (!vertex_code) {
            return unexpected(ui_sector_error("generate ui_sector vertex bytecode failed: " + vertex_code.error().message));
        }
        auto vertex_module = device.create_shader_module(RHI::ShaderModuleDesc{
            ...
            .label = "ui sector vertex module",
        });
        if (!vertex_module) {
            return unexpected(Renderer::graphics_error_from_rhi(vertex_module.error(), "create ui sector vertex module"));
        }
        ...
        if (!fragment_code) {
            ...
            return unexpected(ui_sector_error("generate ui_sector fragment bytecode failed: " + fragment_code.error().message));
        }
        auto fragment_module = device.create_shader_module(RHI::ShaderModuleDesc{
            ...
            .label = "ui sector fragment module",
        });
        if (!fragment_module) {
            pipeline.destroy(device);
            return unexpected(Renderer::graphics_error_from_rhi(fragment_module.error(), "create ui sector fragment module"));
        }
        ...
        if (generated.empty()) {
            pipeline.destroy(device);
            return unexpected(ui_sector_error("ui_sector shader produced no bind-group layout."));
        }
        for (const Renderer::GeneratedBindGroupLayout &layout : generated) {
            auto handle = device.create_bind_group_layout(RHI::BindGroupLayoutDesc{
                ...
                .label = "ui sector bind group layout",
            });
            if (!handle) {
                pipeline.destroy(device);
                return unexpected(Renderer::graphics_error_from_rhi(handle.error(), "create ui sector bind group layout"));
            }
        ...
        if (!pipeline.instances_binding_.found) {
            pipeline.destroy(device);
            return unexpected(ui_sector_error("ui_sector shader reflection did not produce the expected instances binding."));
        }
        ...
        if (push_constant_ranges.empty()) {
            pipeline.destroy(device);
            return unexpected(ui_sector_error("ui_sector shader reflection did not produce the expected viewConstants push-constant range."));
        }
        auto pipeline_layout = device.create_pipeline_layout(RHI::PipelineLayoutDesc{
            ...
            .label = "ui sector pipeline layout",
        });
        if (!pipeline_layout) {
            pipeline.destroy(device);
            return unexpected(Renderer::graphics_error_from_rhi(pipeline_layout.error(), "create ui sector pipeline layout"));
        }
        ...
        const RHI::RenderPipelineDesc desc{
            ...
            .label = "ui sector pipeline",
        };
        auto rhi_pipeline = device.create_render_pipeline(desc);
        if (!rhi_pipeline) {
            pipeline.destroy(device);
            return unexpected(Renderer::graphics_error_from_rhi(rhi_pipeline.error(), "create ui sector pipeline"));
        }
    ...
    Core::RendererResult UiSectorPipeline::prepare(RHI::RhiDevice &device, span<const UiSectorInstance> instances,
                                                   span<const RHI::Rect2D> instance_scissors,
                                                   span<const u32> instance_paint_groups,
                                                   UiSectorFrameResources &resources, vector<UiSectorDrawBatch> &out_batches) {
        out_batches.clear();
        if (instances.size() != instance_scissors.size() || instances.size() != instance_paint_groups.size()) {
            return Core::graphics_backend_error(
                ...
                "UiSectorPipeline::prepare: instance/scissor/paint-group counts must match.");
        }
        if (instances.empty()) {
            return {};
        }
        ...
        while (i < instances.size()) {
            ...
            out_batches.push_back(UiSectorDrawBatch{
        ...
        const u64 required_bytes = static_cast<u64>(instances.size()) * sizeof(UiSectorInstance);
        bool buffer_replaced = false;
        if (!resources.instance_buffer || resources.instance_capacity_bytes < required_bytes) {
            u64 new_capacity = std::max<u64>(sizeof(UiSectorInstance) * 64u, resources.instance_capacity_bytes);
            ...
            auto replacement = device.create_buffer(RHI::BufferDesc{
                ...
                .label = "persistent ui sector instance buffer",
            });
            if (!replacement) {
                out_batches.clear();
                return unexpected(Renderer::graphics_error_from_rhi(replacement.error(), "create persistent ui sector instance buffer"));
            }
            destroy_ui_sector_frame_resources(device, resources);
        ...
        if (!instances_unchanged) {
            ...
            if (auto written = device.write_buffer(resources.instance_buffer, 0,
                ...
                !written) {
                out_batches.clear();
                return unexpected(Renderer::graphics_error_from_rhi(written.error(), "write ui sector instance buffer"));
            }
        ...
        if (buffer_replaced || !resources.bind_groups_valid) {
            ...
            add_entry(instances_binding_, RHI::BindGroupEntry{
                                              ...
                                              .offset = 0,
                                              .size = 0,
                                              .structure_stride = sizeof(UiSectorInstance),
                                          });
            for (const Group &group : groups) {
                auto bind_group = device.create_bind_group(RHI::BindGroupDesc{
                    ...
                    .label = "persistent ui sector bind group",
                });
                if (!bind_group) {
                    for (const UiSectorDrawBatch::BoundGroup &created : resources.bind_groups) {
                        device.destroy_bind_group(created.handle);
                    }
                    resources.bind_groups.clear();
                    out_batches.clear();
                    return unexpected(Renderer::graphics_error_from_rhi(bind_group.error(), "create persistent ui sector bind group"));
                }
                resources.bind_groups.push_back(UiSectorDrawBatch::BoundGroup{
        ...
        for (UiSectorDrawBatch &batch : out_batches) {
    ...
    /// @note Normal failures are returned through the type-specific error/status state; invalid input/state and underlying backend or resource failures are reported there when detected.
    /// @note Error/status alternatives explicitly produced by this implementation include `GraphicsBackendErrorCode::OperationFailed`.
    Core::RendererResult UiSectorPipeline::draw(RHI::RenderPassEncoder &pass, span<const UiSectorDrawBatch> batches,
                                                glm::vec2 viewport_size) {
        if (batches.empty()) {
            return {};
        }
        if (!pipeline_) {
            return Core::graphics_backend_error(Core::GraphicsBackendErrorCode::OperationFailed, "UI sector pipeline was not created.");
        }

        pass.set_pipeline(pipeline_);

        for (const UiSectorDrawBatch &batch : batches) {
            if (!batch.instance_buffer) {
                return Core::graphics_backend_error(Core::GraphicsBackendErrorCode::OperationFailed,
                                                    "UI sector draw batch has no prepared instance buffer.");
            }
            pass.set_scissor(batch.scissor);
            for (const UiSectorDrawBatch::BoundGroup &group : batch.bind_groups) {
                if (!group.handle) {
                    return Core::graphics_backend_error(Core::GraphicsBackendErrorCode::OperationFailed,
                                                        "UI sector draw batch has an invalid persistent bind group.");
                }
    ...
    /// @return Returns the value produced by the operation.
    /// @note This function does not throw exceptions.
    void UiSectorPipeline::destroy(RHI::RhiDevice &device) noexcept {
        if (pipeline_) {
