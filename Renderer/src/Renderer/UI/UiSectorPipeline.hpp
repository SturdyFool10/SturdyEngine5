...
namespace SFT::UI {
    ...
    struct UiSectorFrameResources {
        RHI::BufferHandle instance_buffer{};
        u64 instance_capacity_bytes = 0;
        vector<UiSectorInstance> uploaded_instances;


        vector<UiSectorDrawBatch::BoundGroup> bind_groups;
    ...
    // Untextured sibling of UiQuadPipeline/UiStrokePipeline — draws annular sectors ("pie slices")
    // instead of rects or capsules. See Shaders/ui_sector.slang's own doc comment for the instancing/
    // AA model.
    class UiSectorPipeline {
      public:
        /// Constructs a `UiSectorPipeline` in its default state.
        ///
        /// @note This function does not throw exceptions.
        UiSectorPipeline() noexcept = default;

        /// Creates a `UiSectorPipeline` resource or value from the supplied parameters.
        ///
        /// @param device Device used or affected by the operation.
        ...
        [[nodiscard]] static Core::RendererExpected<UiSectorPipeline> create(
        ...
        [[nodiscard]] Core::RendererResult prepare(RHI::RhiDevice &device, span<const UiSectorInstance> instances,
        ...
        /// @note Normal failures are returned through the type-specific error/status state; invalid input/state and underlying backend or resource failures are reported there when detected.
        /// @note Error/status alternatives explicitly produced by this implementation include `GraphicsBackendErrorCode::OperationFailed`.
        [[nodiscard]] Core::RendererResult draw(RHI::RenderPassEncoder &pass, span<const UiSectorDrawBatch> batches,
                                                 glm::vec2 viewport_size);

        /// Destroys or releases the `UiSectorPipeline` resource represented by the supplied parameters.
        ///
        /// @param device Device used or affected by the operation.
        ///
        /// @note This function does not throw exceptions.
