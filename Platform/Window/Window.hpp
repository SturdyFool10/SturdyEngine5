...
struct WindowError {
    ...
    constexpr WindowError(WindowErrorCode error_code, const char* error_message) noexcept
        : code(error_code)
        , message()
    {
        try {
    ...
    constexpr WindowError(WindowErrorCode error_code, std::string_view error_message) noexcept
        : code(error_code)
        , message()
    {
        try {
...
public:
    ...
    template <typename Backend, typename... Args>
        requires std::derived_from<Backend, Window>
            && requires(Args&&... args) {
