...
[[nodiscard]] inline RendererExpected<CalibratedClocks> get_calibrated_clocks(
    VkDevice device,
    VkTimeDomainKHR cpu_domain = VK_TIME_DOMAIN_CLOCK_MONOTONIC_RAW_KHR
) noexcept {
    VkCalibratedTimestampInfoKHR infos[2]{
        { .sType = VK_STRUCTURE_TYPE_CALIBRATED_TIMESTAMP_INFO_KHR,
          .pNext = nullptr, .timeDomain = VK_TIME_DOMAIN_DEVICE_KHR },
        { .sType = VK_STRUCTURE_TYPE_CALIBRATED_TIMESTAMP_INFO_KHR,
          .pNext = nullptr, .timeDomain = cpu_domain },
    };
    u64 timestamps[2]{};
    u64 max_deviation = 0;
...
class VulkanTimestampPool {
  public:
    ...
    [[nodiscard]] static RendererExpected<VulkanTimestampPool> create(
        VkDevice device, u32 query_count
    ) noexcept {
    ...
    [[nodiscard]] RendererExpected<vector<u64>> resolve(
        u32 first_query = 0, u32 count = 0,
        VkQueryResultFlags flags = VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT
    ) const {
        if (count == 0) count = query_count_;
        vector<u64> ticks(count, 0);
        VkResult res = vkGetQueryPoolResults(device_, pool_, first_query, count,
                                             ticks.size() * sizeof(u64), ticks.data(),
                                             sizeof(u64), flags);
