...
class VulkanDevice {
  public:
    ...
    VulkanDevice(VulkanDevice&& o) noexcept
        : device_(o.device_), physical_device_(o.physical_device_),
          graphics_queue_(o.graphics_queue_), present_queue_(o.present_queue_),
          compute_queue_(o.compute_queue_),   transfer_queue_(o.transfer_queue_) {
    ...
    [[nodiscard]] static RendererExpected<VulkanDevice> create(
        ...
        auto push_unique = [&](optional<u32> fam) {
            if (!fam) return;
            for (u32 f : families) if (f == *fam) return;
        ...
        auto get_queue = [&](optional<u32> fam) -> optional<Queue> {
            if (!fam) return {};
            Queue q{ .family_index = *fam };
            vkGetDeviceQueue(vk_device, *fam, 0, &q.handle);
            return q;
        };
    ...
    [[nodiscard]] const optional<Queue>& graphics_queue() const noexcept { return graphics_queue_; }
    [[nodiscard]] const optional<Queue>& present_queue()  const noexcept { return present_queue_; }
    [[nodiscard]] const optional<Queue>& compute_queue()  const noexcept { return compute_queue_; }
    [[nodiscard]] const optional<Queue>& transfer_queue() const noexcept { return transfer_queue_; }
    ...
    [[nodiscard]] RendererExpected<void*> map_memory(VkDeviceMemory memory, VkDeviceSize offset,
                                                     VkDeviceSize size, VkMemoryMapFlags flags = 0) noexcept {
        void* ptr = nullptr;
    ...
    [[nodiscard]] RendererResult bind_buffer_memory(VkBuffer buffer, VkDeviceMemory memory,
    ...
    [[nodiscard]] RendererResult bind_image_memory(VkImage image, VkDeviceMemory memory,
                                                   VkDeviceSize offset = 0) noexcept {
    ...
    [[nodiscard]] RendererExpected<VkPipeline> create_graphics_pipeline(
        VkPipelineCache cache, const VkGraphicsPipelineCreateInfo& info) noexcept {
    ...
    [[nodiscard]] RendererExpected<vector<VkPipeline>> create_graphics_pipelines(
        VkPipelineCache cache, span<const VkGraphicsPipelineCreateInfo> infos) {
        vector<VkPipeline> pipelines(infos.size(), VK_NULL_HANDLE);
        if (vkCreateGraphicsPipelines(device_, cache, static_cast<u32>(infos.size()),
                                      infos.data(), nullptr, pipelines.data()) != VK_SUCCESS)
    ...
    [[nodiscard]] RendererExpected<VkPipeline> create_compute_pipeline(
        VkPipelineCache cache, const VkComputePipelineCreateInfo& info) noexcept {
        VkPipeline pipeline = VK_NULL_HANDLE;
    ...
    [[nodiscard]] RendererExpected<vector<VkPipeline>> create_compute_pipelines(
        VkPipelineCache cache, span<const VkComputePipelineCreateInfo> infos) {
        vector<VkPipeline> pipelines(infos.size(), VK_NULL_HANDLE);
        if (vkCreateComputePipelines(device_, cache, static_cast<u32>(infos.size()),
                                     infos.data(), nullptr, pipelines.data()) != VK_SUCCESS)
    ...
    [[nodiscard]] VkDescriptorSetLayoutSupport descriptor_set_layout_support(
        ...
        VkDescriptorSetLayoutSupport support{
            .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_SUPPORT, .pNext = nullptr };
    ...
    [[nodiscard]] RendererResult wait_for_fences(span<const VkFence> fences, bool wait_all,
                                                 u64 timeout_ns) noexcept {
        VkResult res = vkWaitForFences(device_, static_cast<u32>(fences.size()), fences.data(),
    ...
    [[nodiscard]] RendererResult get_query_pool_results(VkQueryPool pool, u32 first_query, u32 query_count,
                                                        span<u8> data, VkDeviceSize stride,
                                                        VkQueryResultFlags flags) noexcept {
        VkResult res = vkGetQueryPoolResults(device_, pool, first_query, query_count,
                                             data.size_bytes(), data.data(), stride, flags);
    ...
    // vkQueueSubmit2 requires Vulkan 1.3 / VK_KHR_synchronization2.
    [[nodiscard]] RendererResult queue_submit(VkQueue queue, span<const VkSubmitInfo2> submits,
                                               VkFence fence = VK_NULL_HANDLE) noexcept {
        if (vkQueueSubmit2(queue, static_cast<u32>(submits.size()), submits.data(), fence) != VK_SUCCESS)
            return renderer_error(RendererErrorCode::OperationFailed, "vkQueueSubmit2 failed.");
        return {};
    }

    // VK_SUBOPTIMAL_KHR is treated as success — caller marks the surface dirty.
    [[nodiscard]] RendererResult queue_present(VkQueue queue, const VkPresentInfoKHR& info) noexcept {
    ...
    [[nodiscard]] RendererResult queue_wait_idle(VkQueue queue) noexcept {
        if (vkQueueWaitIdle(queue) != VK_SUCCESS)
            return renderer_error(RendererErrorCode::OperationFailed, "vkQueueWaitIdle failed.");
        return {};
    }

    [[nodiscard]] RendererResult queue_bind_sparse(VkQueue queue, span<const VkBindSparseInfo> infos,
                                                    VkFence fence = VK_NULL_HANDLE) noexcept {
        if (vkQueueBindSparse(queue, static_cast<u32>(infos.size()), infos.data(), fence) != VK_SUCCESS)
    ...
    void set_debug_tag(VkObjectType type, u64 object_handle, u64 tag_name,
                       span<const u8> tag_data) noexcept {
  ...
  private:
    ...
    optional<Queue> graphics_queue_{};
    optional<Queue> present_queue_{};
    optional<Queue> compute_queue_{};
    optional<Queue> transfer_queue_{};
};
