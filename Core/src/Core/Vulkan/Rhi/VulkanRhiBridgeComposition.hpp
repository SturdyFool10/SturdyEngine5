...
namespace SFT::Core::Vulkan {
    ...
    // Vulkan-side half of composition presentation (see graphicsPlatform's CompositionPresent.hpp for
    // the full rationale). Windows starts every supported swapchain generation on this path so a later
    // transparency toggle cannot mix native WSI with DirectComposition. This header declares the seam
    // VulkanRhiBridgeSwapchain.cpp calls into; VulkanRhiBridgeComposition.cpp provides the real
    // Windows implementation and a same-signature non-Windows stub, mirroring exactly how
    // graphicsPlatform itself splits CompositionPresent.cpp / CompositionPresentUnsupported.cpp — two
    // always-compiled translation units, each internally guarded, so call sites here never need an
    // `#if defined(_WIN32)` of their own.
