...
#include <WindowManager/Providers/SDL3/Window.hpp>
...
namespace SFT::WindowManager::SDL3 {
    namespace {
    ...
    SDL3Window::~SDL3Window() noexcept {
        ...
        if (window_) [[likely]] {
            ...
            if (auto native = ::SFT::WindowManager::Detail::native_window_handle_from_sdl(window_)) {
    ...
    optional<WindowHdrProperties> SDL3Window::hdr_properties() const noexcept {
        ...
        if (refresh_wayland_reference) {
            ...
            if (auto handle = ::SFT::WindowManager::Detail::native_window_handle_from_sdl(window_);
                handle && handle->system == NativeWindowSystem::Wayland) {
                wayland_reference_white_nits_ =
                    ::SFT::WindowManager::Detail::query_wayland_surface_reference_white(*handle);
            }
        }
    ...
    expected<NativeWindowHandle, WindowError> SDL3Window::native_window_handle_locked() const noexcept {
        ...
        auto handle = ::SFT::WindowManager::Detail::native_window_handle_from_sdl(window_);
        if (!handle) [[unlikely]] {
