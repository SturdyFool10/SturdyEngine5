...
namespace SFT::Text {
    ...
    class Anchor {
      public:
        ...
        [[nodiscard]] AnchorBias bias() const noexcept { return bias_; }
    ...
    class EditTransaction {
      public:
        explicit EditTransaction(Revision base_revision) : base_revision_(base_revision) {}
        void replace(TextRange range, string_view replacement) { splices_.push_back({range, replacement}); }
        [[nodiscard]] Revision base_revision() const noexcept { return base_revision_; }
        [[nodiscard]] const vector<Splice> &splices() const noexcept { return splices_; }
    ...
    class TextSlice {
      public:
        ...
        [[nodiscard]] const vector<Chunk> &chunks() const noexcept { return chunks_; }
        [[nodiscard]] usize byte_size() const noexcept { return byte_size_; }
    ...
    class DocumentSnapshot {
      public:
        ...
        // --- IDE-shaped primitives. All of it is plain-text/structural: no tokenizer, no grammar,
        // no LSP client — this is the vocabulary an editor UI (selection, indentation, bracket
        // matching, find) needs from the buffer itself, independent of any language intelligence
      ...
      private:
        friend class Document;
        explicit DocumentSnapshot(shared_ptr<const Detail::DocumentState> state) : state_(std::move(state)) {}
