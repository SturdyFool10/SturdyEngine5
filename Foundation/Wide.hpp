...
#error "SturdyEngine's f128/f256 require IEEE FP semantics; -ffast-math/-Ofast breaks them."
    ...
    class u256 {
      public:
        ...
        constexpr u256 operator-() const noexcept { return ~*this + u256{1}; } // modular negation, like a built-in unsigned
    ...
    class i256 {
      public:
        ...
        // Implicit, like a built-in `unsigned = signed`: the two's-complement bits reinterpret directly,
        // so in mixed i256/u256 arithmetic the unsigned type wins (matching the usual arithmetic conversions).
    ...
    namespace Detail {
        ...
        // Which scalar kinds widen into an expansion float (f128/f256): every built-in integer (incl.
        ...
        [[nodiscard]] constexpr TwoF64 quick_two_sum(f64 a, f64 b) noexcept { // requires |a| >= |b|
            const f64 s = a + b;
            return {s, b - (s - a)};
        }
        ...
        [[nodiscard]] constexpr TwoF64 split_product_operand(f64 a) noexcept {
            constexpr f64 splitter = 134217729.0; // 2^27 + 1: Dekker split for binary64.
    ...
    class f128 {
      public:
        ...
        // Any integer (built-in or wide) widens in, like `double = someInt`; the full value is carried.
    ...
    namespace Detail {
        ...
        [[nodiscard]] consteval bool wide_conversion_smoke_test() noexcept {
            // Built-in and wide integers widen implicitly into the expansion floats (like `double = int`).
            if (static_cast<f64>(f256(42)) != 42.0 || static_cast<f64>(f128(-7)) != -7.0)
                return false;
            if (static_cast<f64>(f256(i256((i128)-123456789))) != -123456789.0)
                return false;

            // The widening is precise past f64: a 101-bit value keeps its low bit through f256.
            ...
            // Mixed arithmetic now resolves to the wider type with no cast — the reported f256 - i256 case.
            if (static_cast<f64>(f256(1000) - i256((i128)1)) != 999.0)
                return false;

            // Signed 256 widens into unsigned 256 implicitly (unsigned wins, as in the usual conversions).
    ...
    //
    // Pull these in with `using namespace SFT::Foundation::Literals;` so the wide types read as
    // naturally as the built-ins:
    //     auto big  = 340282366920938463463374607431768211455_u128; // 2^128 - 1, full width
    //     auto mask = 0xFFFF'0000'FFFF'0000_u256;                   // bases + ' separators
    //     auto k    = 3.14159265358979323846264338327950288_f256;  // parsed straight into qd
    //
    // The raw (const char*) form is deliberate: a cooked `unsigned long long`/`long double` literal
    // would truncate the value to 64-ish bits before the wide type ever saw it. Parsing the literal
    // text instead keeps the full width for the integers and very near full precision for the floats.

    namespace Detail {

        // Decimal / 0x.. / 0b.. / 0.. (octal) into the unsigned wide accumulator U, ' separators ok.
        template <class U>
        ...
        // value * 10^exponent, the scale factor built in-type by binary exponentiation.
        template <class F>
        ...
        // [digits][.digits][(e|E)[+|-]digits] into the expansion-float type F, ' separators ok.
        template <class F>
    ...
    namespace Detail {
        ...
        [[nodiscard]] consteval bool wide_literal_smoke_test() noexcept {
            ...
            if (static_cast<u64>(255_u128) != 255u || static_cast<u64>(0xFF_u128) != 255u)
                return false;
            if (static_cast<u64>(1'000_u128) != 1000u) // ' digit separator
                return false;
            if (static_cast<u64>(0b1010_u256) != 10u || static_cast<u64>(010_u256) != 8u) // binary + leading-zero octal
                return false;
            if (static_cast<i64>(-7_i128) != -7) // unary minus applies to the literal's value
                return false;
            if (static_cast<bool>(1_u256 + -(1_u256))) // u256 modular negation cancels
                return false;
