...
namespace SFT::Foundation::Cpu {
    ...
    struct CpuFeatures {
        ...
        char vendor[13] = {}; // x86 only: e.g. "GenuineIntel", "AuthenticAMD" -- leaf 0 EBX/EDX/ECX
        char brand[49] = {};  // x86 only: processor brand string from leaves 0x80000002-4, if reported

        [[nodiscard]] std::string_view vendor_view() const noexcept { return vendor; }
        [[nodiscard]] std::string_view brand_view() const noexcept { return brand; }
    };
