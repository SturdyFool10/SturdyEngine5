...
// Descriptor storage for the D3D12 backend, split the way D3D12 itself splits it.
//
// D3D12 descriptors live in heaps, and heaps come in two flavours with very different rules:
//
//  - *Non-shader-visible* (CPU) heaps are ordinary CPU memory. Descriptors can be created into them
//    freely at any time, they can be arbitrarily many, and they cost nothing to keep around. This is
//    where every RTV, DSV, and every bind group's *authoritative* descriptors live. A bind group is
//    created once and must stay valid for its whole lifetime, so it needs storage that is never
//    recycled underneath it — that is `CpuDescriptorAllocator`.
//
//  - *Shader-visible* (GPU) heaps are what a shader actually reads through, and only **one** heap of
//    each type may be bound to a command list at a time. They are also a bounded resource (a sampler
//    heap tops out at 2048 descriptors on every tier). So they cannot be the authoritative storage;
//    they are a per-command-list scratch area that bind groups are *copied into* at bind time, which
//    is `ShaderVisibleDescriptorHeap`.
//
// That copy-at-bind-time design is the standard, correct D3D12 shape and is what makes the RHI's
// "create a bind group once, bind it anywhere" contract implementable at all. The alternative —
// creating bind groups directly in a shader-visible heap — would make a bind group's validity depend
// on which heap happened to be bound, which the RHI does not (and should not) expose.
...
namespace SFT::D3D12 {
    ...
    class CpuDescriptorAllocator {
      public:
        ...
        [[nodiscard]] u32 increment() const noexcept { return increment_; }
        [[nodiscard]] D3D12_DESCRIPTOR_HEAP_TYPE heap_type() const noexcept { return type_; }
    ...
    class ShaderVisibleDescriptorHeap {
      public:
        ...
        [[nodiscard]] bool is_valid() const noexcept { return heap_ != nullptr; }
        [[nodiscard]] u32 increment() const noexcept { return increment_; }
