...
// Startup-time physical-GPU discovery across all registered graphics APIs. The inventory is a snapshot:
// it owns no native adapter/instance handles, so callers may collect it before renderer startup and retain
// it for a settings UI, diagnostics, or backend selection without keeping API loaders alive.
